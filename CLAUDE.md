...
### Claude Code環境設定
**Claude Code環境はWindows環境に切り替え済み**。以下の点を確認：
- パス指定時は `/mnt/d/` ではなく `D:\` または `D:/` を使用
- ファイル探索・実行時もWindows形式パスで処理
- WSL2マウントパスは使用しない
...
**テストファイルの管理**: 今後作成するすべてのテストファイルは`test`フォルダ内に配置すること。

### テストファイル配置ルール
- テストファイルは必ず`D:\setsuna_bot\test\`ディレクトリに作成
- テストファイル名は`test_`で始める
- 機能別にサブディレクトリを作成可能（例: `test\gui\`, `test\voice\`）
- 実行時もtestフォルダ内から実行する
...
この方針により、テストファイルの整理とメンテナンスが容易になる。